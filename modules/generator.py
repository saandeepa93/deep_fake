...
class Generator(nn.Module):
  def __init__(self, in_features, out_features, max_features, num_blocks, block_expansion,\
    kp_variance, norm_const, use_heatmaps, use_deformed_source_image, heatmap_type):
    super(Generator, self).__init__()

    self.appearance_encoder = Encoder(in_features, out_features, max_features,\
      num_blocks, block_expansion)
    self.densemotion = DenseMotionModule(in_features, out_features, kp_variance,\
      block_expansion, num_blocks, norm_const, use_heatmaps, use_deformed_source_image, heatmap_type)


  def forward(self, x, kp_source_mean, kp_driving_mean):
    appearance_skips = self.appearance_encoder(x)
    print("appearance skips: ", appearance_skips[-1].size())
